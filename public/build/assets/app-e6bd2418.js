...
</div>`},sv={name:"VerifyEmail",data(){return{status:"verifying",message:""}},async mounted(){var r,s;const{id:e,hash:t}=this.$route.params;if(!e||!t){this.status="error",this.message="Invalid verification link.";return}try{const{data:a}=await this.$http.get(`/auth/verify-email/${e}/${t}`);if(this.$auth.isLoggedIn)try{const i=await this.$http.get("/user");this.$auth.setUser(i.data.data)}catch{}a.message&&a.message.toLowerCase().includes("already")?this.status="already":this.status="success"}catch(a){this.status="error",this.message=((s=(r=a.response)==null?void 0:r.data)==null?void 0:s.message)||"Verification failed. The link may have expired."}},methods:{proceed(){this.$auth.isLoggedIn?this.$router.push("/onboarding"):this.$router.push("/login")}},template:`
...
<div class="min-h-screen bg-gray-50">
  ...
  <app-footer />
</div>`},Gv={name:"HowItWorks",template:`
<div class="min-h-screen bg-gray-50">
...
</div>`},Yv={name:"SafetyAndEscrow",template:`
<div class="min-h-screen bg-gray-50">
  <nav class="bg-white border-b border-gray-200 sticky top-0 z-40">
    <div class="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
      <router-link to="/" class="flex items-center gap-2">
...
</div>`},r0={props:{size:{default:"md"}},template:`<div class="flex justify-center items-center py-8">
...
</div>`},s0={props:{type:{default:"info"},message:String,dismissible:{default:!0}},data(){return{visible:!0}},computed:{classes(){return{info:"bg-blue-50 border-blue-200 text-blue-800",success:"bg-green-50 border-green-200 text-green-800",warning:"bg-yellow-50 border-yellow-200 text-yellow-800",error:"bg-red-50 border-red-200 text-red-800"}[this.type]||"bg-gray-50 border-gray-200 text-gray-800"},icon(){return{info:"fa-info-circle",success:"fa-check-circle",warning:"fa-exclamation-triangle",error:"fa-times-circle"}[this.type]||"fa-info-circle"}},template:`<div v-if="visible && message" :class="['flex items-start gap-3 p-4 rounded-lg border text-sm', classes]">
...
</div>`},c0={props:{meta:Object},template:`<div v-if="meta && meta.last_page > 1" class="flex items-center justify-between mt-6">
...
</div>`},d0={template:`<footer class="bg-white border-t border-gray-100 mt-16 py-8">
