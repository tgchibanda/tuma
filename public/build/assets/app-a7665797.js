...
</div>`},sv={name:"Register",data(){return{form:{first_name:"",last_name:"",email:"",phone:"",password:"",password_confirmation:"",country_id:1,referral_code:"",captcha_answer:"",captcha_token:""},loading:!1,error:null,captcha:{question:"",answer:0,token:""}}},async created(){this.$route.query.ref&&(this.form.referral_code=this.$route.query.ref),this.newCaptcha()},methods:{newCaptcha(){const e=Math.floor(Math.random()*7)+3,t=Math.floor(Math.random()*7)+2,s=[{label:`${e} + ${t}`,ans:e+t},{label:`${e} × ${t}`,ans:e*t},{label:`${e+t} − ${t}`,ans:e}],r=s[Math.floor(Math.random()*s.length)],a=Math.random().toString(36).slice(2);this.captcha={question:`What is ${r.label}?`,answer:r.ans,token:btoa(r.ans+":"+a),salt:a},this.form.captcha_answer="",this.form.captcha_token=this.captcha.token},captchaValid(){return parseInt(this.form.captcha_answer)===this.captcha.answer},async submit(){var e,t,s,r;if(!this.form.captcha_answer){this.error="Please answer the security question.";return}if(!this.captchaValid()){this.error="Incorrect answer. Please try again.",this.newCaptcha();return}this.loading=!0,this.error=null;try{const{data:a}=await this.$http.post("/auth/register",this.form);this.$auth.login(a.data.token,a.data.user),this.$router.push("/onboarding")}catch(a){const i=(t=(e=a.response)==null?void 0:e.data)==null?void 0:t.errors;this.error=i?Object.values(i).flat()[0]:((r=(s=a.response)==null?void 0:s.data)==null?void 0:r.message)||"Registration failed.",this.newCaptcha()}this.loading=!1}},template:`<div class="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-8">
  <div class="w-full max-w-sm">
    <div class="text-center mb-8">
