export default {
    ...
    methods: {
        async load(page = 1) {
            ...
            try {
                const { data } = await this.$http.get('/../../api/admin/users', { params })
        ...
        kycBadge(s) {
            return { pending: 'bg-gray-100 text-gray-600', submitted: 'bg-blue-100 text-blue-700', approved: 'bg-green-100 text-green-700', rejected: 'bg-red-100 text-red-700' }[s] || 'bg-gray-100 text-gray-600'
        },
        statusBadge(s) {
            return { active: 'bg-green-100 text-green-700', suspended: 'bg-orange-100 text-orange-700', banned: 'bg-red-100 text-red-700' }[s] || 'bg-gray-100 text-gray-600'
        }
    },
    template: `
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">KYC</th>
              <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Status</th>
              <th class="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Trades</th>
              <th class="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Trust</th>
            ...
            <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 transition">
              <td class="px-4 py-3">
                <div>
                  ...
                  <p v-if="user.report_count > 0" class="text-xs text-red-500 mt-0.5">
                    <i class="fas fa-flag mr-0.5"></i>{{ user.report_count }} report(s)
                  </p>
                </div>
              </td>
              <td class="px-4 py-3">
                <span :class="['text-xs font-medium px-2 py-0.5 rounded-full capitalize', kycBadge(user.kyc_status)]">
                  {{ user.kyc_status }}
                </span>
              </td>
              <td class="px-4 py-3">
                <span :class="['text-xs font-medium px-2 py-0.5 rounded-full capitalize', statusBadge(user.account_status)]">
                  {{ user.account_status }}
                </span>
              </td>
              <td class="px-4 py-3 text-center text-gray-700">{{ user.successful_trades }}</td>
              <td class="px-4 py-3 text-center">
                <span :class="['text-xs font-bold', user.trust_score >= 70 ? 'text-green-600' : user.trust_score >= 40 ? 'text-yellow-600' : 'text-red-500']">
                  {{ user.trust_score }}
                </span>
              </td>
              <td class="px-4 py-3 text-xs text-gray-500">{{ $fmt.date(user.created_at) }}</td>
              <td class="px-4 py-3">
                <div class="flex items-center gap-1">
                  <router-link :to="'/admin/users/' + user.id"
                    class="px-2 py-1 text-xs text-green-700 border border-green-200 rounded-lg hover:bg-green-50">View</router-link>
                  <button v-if="user.kyc_status === 'submitted'" @click="approveKyc(user.id)"
                    :disabled="actionLoading === user.id + '_kyc'"
                    class="px-2 py-1 text-xs text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-50">
                    Approve KYC
                  </button>
                  <button v-if="user.kyc_status === 'submitted'" @click="rejectKyc(user.id)"
                    :disabled="actionLoading === user.id + '_kyc'"
                    class="px-2 py-1 text-xs text-red-600 border border-red-200 rounded-lg hover:bg-red-50">
                    Reject
                  </button>
                  <button v-if="user.account_status === 'active'" @click="suspend(user.id)"
                    :disabled="actionLoading === user.id + '_suspend'"
                    class="px-2 py-1 text-xs text-orange-600 border border-orange-200 rounded-lg hover:bg-orange-50">
                    Suspend
                  </button>
                </div>
              </td>
